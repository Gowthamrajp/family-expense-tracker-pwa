...
export interface ExpenseRepository {
  ...
  /**
   * Delete all expenses that were generated by the given recurring rule
   * (matched on `recurringRuleId`). Used when a member chooses to remove a
   * recurring payment's previously-generated transactions. Returns the number
   * of expenses deleted. Batched for efficiency.
   */
  deleteExpensesByRecurringRule(
    familyId: string,
...
function readDocument(snapshot: QueryDocumentSnapshot<DocumentData>): Expense {
  ...
  if (data.recurringRuleId !== undefined) {
    doc.recurringRuleId = data.recurringRuleId;
  }
...
export const expenseRepository: ExpenseRepository = {
  async addExpense(
    ...
    if (docData.recurringRuleId === undefined) {
      delete docData.recurringRuleId;
    }
  ...
  async deleteExpensesByRecurringRule(
    familyId: string,
    recurringRuleId: string,
  ): Promise<number> {
    // Find every expense generated by this rule and remove them in batches.
    // Firestore caps a write batch at 500 ops, so chunk accordingly.
